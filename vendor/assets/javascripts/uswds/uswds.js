...
var fileInput = behavior({}, {
  init: function init(root) {
    select(DROPZONE, root).forEach(function (fileInputEl) {
      constructFileInput(fileInputEl);
    });
  }
});
module.exports = fileInput;

$(document).arrive("." + DROPZONE_CLASS, function(newElem) {
    if ($(newElem).children().length === 0) {
      //the new file input when the form was cleared
      constructFileInput(newElem);
    } else {
      // the cloned file input
      var newElemFileInput = $(newElem).find('input')[0];
      if (newElemFileInput.onchange === undefined || newElemFileInput.onchange === null) {
        var dropTarget = $(newElem).find('.' + TARGET_CLASS)[0];
        var instructions = $(newElem).find('.' + INSTRUCTIONS_CLASS)[0];
        attachFileInputListeners(newElemFileInput, dropTarget, instructions);
      }
    }
});
